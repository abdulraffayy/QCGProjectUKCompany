...
import { QAQF_LEVELS, MODULE_TYPE_OPTIONS } from "../types";
...
const LessonPlanPage: React.FC = () => {
  ...
  const [editModuleDialogWeekId, setEditModuleDialogWeekId] = useState<
    number | null
  >(null);
  const [editModuleDialogModuleId, setEditModuleDialogModuleId] = useState<
    number | null
  >(null);
  ...
  const [editModuleDialogDescription, setEditModuleDialogDescription] =
    useState("");
  ...
  const [newModuleQAQF, setNewModuleQAQF] = useState<string>("");
  ...
  const [rightSideDeleteDialogOpen, setRightSideDeleteDialogOpen] =
    useState(false);
  ...
  const [editModuleSelectedText, setEditModuleSelectedText] =
    useState<string>("");
  const [editModuleIsPopupOpen, setEditModuleIsPopupOpen] =
    useState<boolean>(false);
  const [editModuleIsLoading, setEditModuleIsLoading] =
    useState<boolean>(false);
  const [
    editModuleCurrentExplanationType,
    setEditModuleCurrentExplanationType,
  ] = useState<string>("explain");
  const [editModuleAiResponse, setEditModuleAiResponse] = useState<string>("");
  const [editModuleShowTooltip, setEditModuleShowTooltip] =
    useState<boolean>(false);
  const [editModuleTooltipPosition, setEditModuleTooltipPosition] = useState({
    x: 0,
    y: 0,
  });
  const [editModuleAiQuery, setEditModuleAiQuery] = useState<string>("");
  const [editModuleAiReference, setEditModuleAiReference] =
    useState<string>("");
  const [editModuleIsAIGenerating, setEditModuleIsAIGenerating] =
    useState<boolean>(false);
  const [editModuleResponseHistory, setEditModuleResponseHistory] = useState<
    Array<{ type: string; content: string; timestamp: number }>
  >([]);
  const [editModuleSelectedLinePosition, setEditModuleSelectedLinePosition] =
    useState<number>(-1);
  const [editModuleIsEditorReady, setEditModuleIsEditorReady] =
    useState<boolean>(false);
  ...
  useEffect(() => {
    if (rightSideEditDialogOpen) {
      ...
      console.log(
        "Edit dialog opened with description:",
  ...
  const [currentExplanationType, setCurrentExplanationType] =
    useState<string>("explain");
  ...
  const [responseHistory, setResponseHistory] = useState<
    Array<{ type: string; content: string; timestamp: number }>
  >([]);
  const [selectedQAQFLevel, setSelectedQAQFLevel] = useState<string>("");
  ...
  // Sync selectedQAQFLevel with newModuleQAQF when it changes
  useEffect(() => {
    if (newModuleQAQF) {
      setSelectedQAQFLevel(newModuleQAQF);
    }
  }, [newModuleQAQF]);
    ...
    if (!isEditorReady || !tiptapEditorRef.current.isReady()) {
      ...
      setTimeout(() => {
        if (
          ...
          tiptapEditorRef.current.isReady() &&
          isEditorReady
        ) {
...
<div class="my-4 bg-gradient-to-r from-blue-50 to-purple-50 border-l-4 border-indigo-500 p-4 rounded-lg shadow-md">
  ...
  <div class="mt-3 text-gray-700">
    ...
    try {
      ...
      const contentLength =
        tiptapEditorRef.current.getEditor().state.doc.content.size;
      ...
      const result = tiptapEditorRef.current.insertContent(
      ...
      try {
        ...
        console.log(
          "✅ Fallback: Updated lesson content state (appended to end)"
        );
        toast.success(
          `AI explanation attached at the bottom (fallback method)!`,
          {
            position: "top-right",
            autoClose: 3000,
          }
  ...
  const selectQuafLevelfunction = (level: string) => {
    console.log("level", level);
    setSelectedQAQFLevel(level);
  }
  ...
  const sendExplanationRequest = async () => {
    ...
    try {
      ...
      const generation_type =
        generationTypeMap[currentExplanationType] || "explanation";
      const material = aiReference || selectedText;
      // const qaqf_level = selectedQAQFLevel || ""; // Use selected QAQF level or default to "1"
      const subject = selectedText;
      const userquery =
        aiQuery ||
        `Please ${currentExplanationType} this text: ${selectedText}`;

      const response = await fetch(
        "http://69.197.176.134:5000/api/ai/assessment-content",
        {
          method: "POST",
          headers: {
      ...
      if (data.generated_content && data.generated_content.length > 0) {
        ...
        const formattedResponses = allResponses
          .reverse()
          .map((resp) => {
            return resp.content;
          })
  ...
  const handleEditModuleTextSelection = (
    selectedText: string,
    selectionPosition?: { from: number; to: number }
  ) => {
    if (selectedText && selectedText.trim() !== "") {
  ...
  const sendEditModuleExplanationRequest = async () => {
    ...
    try {
      ...
      const generation_type =
        generationTypeMap[editModuleCurrentExplanationType] || "explanation";
      const material = editModuleAiReference || editModuleSelectedText;
      const subject = editModuleSelectedText;
      const userquery =
        editModuleAiQuery ||
        `Please ${editModuleCurrentExplanationType} this text: ${editModuleSelectedText}`;
      ...
      const response = await fetch(
        "http://69.197.176.134:5000/api/ai/assessment-content",
        {
          ...
          body: JSON.stringify({
            generation_type,
            material,
            qaqf_level: rightSideEditForm.qaqfLevel,
            subject,
            userquery,
  ...
  const attachEditModuleExplanation = () => {
    ...
    console.log(
      "Edit Module - Selected line position:",
      editModuleSelectedLinePosition
    );

    // Check if editor is ready
    if (
      !editModuleTiptapEditorRef.current ||
      !editModuleTiptapEditorRef.current.isReady() ||
...
<div class="my-4 bg-gradient-to-r from-blue-50 to-purple-50 border-l-4 border-indigo-500 p-4 rounded-lg shadow-md">
  ...
  <div class="mt-3 text-gray-700">
    ...
    try {
      // Always insert at the end of the editor content
      const contentLength =
        editModuleTiptapEditorRef.current.getEditor().state.doc.content.size;
      const insertPosition = contentLength;

      console.log(
        "Edit Module - Inserting at end of editor, position:",
        insertPosition
      );

      // Insert content at the end
      const result = editModuleTiptapEditorRef.current.insertContent(
        formattedExplanation,
        insertPosition
      );

      console.log(
        "✅ Edit Module - AI response inserted successfully at the end"
      );
      ...
      try {
        ...
        console.log(
          "✅ Edit Module - Fallback: Updated form state (appended to end)"
        );
        toast.success(
          `AI explanation attached at the bottom (fallback method)!`,
          {
...
<div class="my-4 bg-gradient-to-r from-blue-50 to-purple-50 border-l-4 border-indigo-500 p-4 rounded-lg shadow-md">
  ...
  <div class="mt-3 text-gray-700">
    ...
    try {
      ...
      // Show success message
      toast.success(
        `AI explanation attached to "${selectedText.substring(0, 30)}${
  ...
  const renderLessonContentWithExplanations = () => {
    ...
    console.log(
      "Rendering lesson content with explanations:",
      sortedExplanations
    );
    console.log("Total explanations:", explanations.length);
    console.log(
      "Line positions of explanations:",
      sortedExplanations.map((exp) => exp.position)
    );
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      result.push(
        <div
          key={`line-${i}`}
          className="mb-2"
          dangerouslySetInnerHTML={{ __html: parseMarkdown(lines[i]) }}
        ></div>
      );

      // Check if there are explanations to insert after this line
      while (
        explanationIndex < sortedExplanations.length &&
        sortedExplanations[explanationIndex].position === i
      ) {
        const explanation = sortedExplanations[explanationIndex];
        console.log(
          `Inserting explanation ${
            explanation.id
          } after line ${i} for text: "${explanation.content.substring(
            0,
            50
          )}..."`
        );
            ...
            {!explanation.isCollapsed && (
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: parseMarkdown(explanation.content),
                }}
              ></div>
            )}
          </div>
    ...
    try {
      ...
      toast.error(
        err instanceof Error
          ? err.message
          : "Could not add week. Please try again."
      );
    }
  };
      ...
      if (updatedModuleRes.ok) {
        ...
        setModules((prevModules) =>
          prevModules.map((module) =>
            module.id === id ? { ...module, ...serverUpdatedModule } : module
          )
        );
      } else {
  ...
  const handleSaveRightSideModuleEdit = async () => {
    ...
    try {
      ...
      // Fetch the updated module from the server to get the correct data structure
      const updatedModuleRes = await fetch(
        `/api/lessons/${rightSideEditModule.id}`
      );
      if (updatedModuleRes.ok) {
  ...
  const fetchCourseData = async (courseId: string, skipWeekLessons = false) => {
    ...
    try {
      ...
      const weeksRes = await fetch(
        `/api/weeks?courseid=${encodeURIComponent(courseId)}`
      );
  ...
  const handleDeleteLessonFromWeek = async (
    weekId: number,
    weekLessonId: number
  ) => {
    try {
      ...
      setWeekLessons((prev) => ({
        ...prev,
        [weekId]: Array.isArray(wlData) ? wlData : [],
      }));
    } catch (err) {
  ...
  const [deleteWeekLessonDialogOpen, setDeleteWeekLessonDialogOpen] =
    useState(false);
  const [deleteWeekLessonData, setDeleteWeekLessonData] = useState<{
    weekId: number;
    weekLessonId: number;
  } | null>(null);
    ...
    <div className="container max-w-screen-xl mx-auto py-6 px-4">
      <div className="space-y-6">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <p className="text-neutral-600 mb-4">
            Create and manage weekly lesson plans for your course content.
            Organize your approved content into a structured schedule for up to
            12 weeks.
          </p>
          <div className="border rounded-md p-4 bg-neutral-50 mb-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-sm font-medium">
                Only approved course content can be added to weekly lesson plans
              </p>
            </div>
            <p className="text-sm text-neutral-600">
              Select modules from your library and assign them to weeks using
              the add/edit features below. Drag-and-drop is disabled.
            </p>
          </div>
                ...
                <SelectContent>
                  {courses.length === 0 ? (
                    ...
                    courses.map((course, idx) => (
                      <SelectItem key={idx} value={course.id}>
                        {course.title}
                      </SelectItem>
                    ))
                  )}
                              ...
                              >
                                ...
                                :{" "}
                                {
                                  QAQF_LEVELS[
                                    (content.qaqfLevel ||
                                      content.level ||
                                      content.qaqf_level) as keyof typeof QAQF_LEVELS
                                  ]
                                }
                              </span>
                            )}
                        ...
                        {openModuleId === content.id && (
                          <div className="mt-3 bg-neutral-50 border rounded shadow p-4 animate-slideDown relative">
                            ...
                            <div className="space-y-1 text-sm">
                              ...
                              <div>
                                ...
                                {(() => {
                                  ...
                                  if (
                                    qaqfLevel &&
                                    QAQF_LEVELS[
                                      qaqfLevel as keyof typeof QAQF_LEVELS
                                    ]
                                  ) {
                              ...
                              <div>
                                <b>User ID:</b> {content.userid}
                              </div>
                              <div>
                                <b>Course ID:</b> {content.courseid}
                              </div>
                              <div>
                                <b>Description:</b>
                                <div
                  ...
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex gap-2">
                      <span className="text-sm text-neutral-500">
                        {selectedCourse
                          ? `${weeksFromApi.length} weeks for this course`
                          : "No course selected"}
                      </span>
                      <Button
                        variant="outline"
                      ...
                      >
                        ...
                        <div
                          ...
                          onDrop={async (e) => {
                            ...
                            if (module) {
                              try {
                                const result = await assignModuleToWeek(
                                  module,
                                  week
                                );
                                if (result) {
                                  ...
                                  toast.success(
                                    `${module.title} added to ${week.title}`
                                  );
                                }
                              } catch (error) {
                                console.error(
                                  "Error assigning module to week:",
                                  error
                                );
                        ...
                        >
                          ...
                          {Array.isArray(weekLessons[week.id]) &&
                          weekLessons[week.id].length > 0 ? (
                            <DndContext
                              ...
                              onDragEnd={async (event) => {
                                ...
                                const oldIndex = weekLessons[week.id].findIndex(
                                  (l: any) => l.id === active.id
                                );
                                const newIndex = weekLessons[week.id].findIndex(
                                  (l: any) => l.id === over.id
                                );
                                ...
                                const orderPayload = newLessons.map(
                                  (l: any, idx: number) => ({
                                    id: l.id,
                                    orderno: idx + 1,
                                  })
                                ...
                                const validOrderPayload = orderPayload.filter(
                                  (item) =>
                                    item.id !== undefined &&
                                            ...
                                            <span>
                                              ...
                                              <Button
                                                ...
                                                onClick={(e) => {
                                                  ...
                                                  setDeleteWeekLessonDialogOpen(
                                                    true
                                                  );
                                                  setDeleteWeekLessonData({
                                                    weekId: week.id,
                                                    weekLessonId,
                                                  });
                                                }}
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent>
          ...
          />
          <DialogFooter>
            <Button variant="outline" onClick={handleDialogSave}>
              Save
            </Button>
            <DialogClose asChild>
        ...
        <DialogContent className="fixed left-1/2 bottom-8 top-auto -translate-x-1/2 w-full max-w-md">
          ...
          <div className="space-y-3">
            <input
              ...
              onChange={(e) =>
                setAddWeeksForm((f) => ({ ...f, title: e.target.value }))
              }
          ...
          <DialogFooter>
            <Button variant="outline" onClick={handleAddWeekApi}>
              Add Week
            </Button>
            <DialogClose asChild>
              <Button variant="ghost">Cancel</Button>
      ...
      >
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setEditModuleDialogQAQF(Number(e.target.value))
                }
              >
            ...
            <div className="overflow-auto" style={{ maxHeight: 250 }}>
              <JoditEditor
                ...
                tabIndex={1}
                onBlur={(newContent) =>
                  setEditModuleDialogDescription(newContent)
                }
                onChange={() => {}}
              />
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={handleEditModuleDialogSave}>
              Save
            </Button>
            <DialogClose asChild>
              <Button variant="ghost">Cancel</Button>
                ...
                <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl max-h-[80vh] bg-white rounded-2xl shadow-2xl overflow-hidden z-50 animate-popupIn">
                  ...
                  <div className="overflow-y-auto max-h-[calc(80vh-80px)]">
                    <div className="p-5 border-b border-gray-100 bg-gray-50 rounded-t-lg">
                      ...
                      value={selectedQAQFLevel}
                      onChange={(e) => {
                        selectQuafLevelfunction(e.target.value);
                    ...
                    <div className="p-5 flex flex-wrap gap-2">
                      {["explain", "summary", "detailed", "examples"].map(
                        (type) => (
                          <button
                            key={type}
                ...
                <div className="grid grid-cols-5 gap-4 items-end mt-6">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      {Object.entries(MODULE_TYPE_OPTIONS).map(
                        ([value, label]) => (
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      value={newModuleQAQF}
                      onChange={(e) => {
                        setNewModuleQAQF(e.target.value);
                    ...
                    <div className="absolute top-4 right-4 z-10">
                      <button
                        ...
                        onClick={() => {
                          ...
                          setSelectedQAQFLevel(newModuleQAQF); // Initialize with current QAQF level
              ...
              onClick={async () => {
                try {
                  ...
                  if (!res.ok) {
                    const errorData = await res.json();
                    throw new Error(
                      errorData.message || "Failed to add course"
                    );
                  }
                  ...
                  toast.error(
                    err instanceof Error ? err.message : "Could not add course"
                  );
                }
              }}
            >
            ...
            <Button
              ...
              onClick={async () => {
                if (deleteModuleTarget) {
                  const success = await handleDeleteModuleApi(
                    deleteModuleTarget.moduleId
                  );
                  if (success) {
      ...
      <Dialog
        open={rightSideEditDialogOpen}
        onOpenChange={setRightSideEditDialogOpen}
      >
                    ...
                    <div className="p-5 border-b border-gray-100 bg-gray-50 rounded-t-lg">
                      ...
                      <div className="mt-2">
                        <TiptapEditor
                          ...
                          onContentChange={(newContent) =>
                            setEditModuleSelectedText(newContent)
                          }
                          className="bg-white border border-gray-200 rounded-lg min-h-[100px]"
                        />
                      </div>
...
<select
                      ...
                      onChange={(e) =>
                        setRightSideEditForm((f) => ({
                          ...f,
                          qaqfLevel: e.target.value,
                        }))
                      }
                    >
                      {Object.values(QAQF_LEVELS).map((qaqfLevel) => (
                      ...
                      {["explain", "summary", "detailed", "examples"].map(
                        (type) => (
                          <button
                            ...
                            onClick={() =>
                              setEditModuleCurrentExplanationType(type)
                            }
                          >
                            {type === "explain" && "💡 Explain"}
                            {type === "summary" && "📝 Summary"}
                        ...
                        <div className="mt-5 p-5 bg-gray-50 rounded-xl border border-green-200">
                          ...
                          <div className="text-gray-700 max-h-60 overflow-y-auto">
                            <TiptapEditor
                              ...
                              onContentChange={(newContent) =>
                                setEditModuleAiResponse(newContent)
                              }
      ...
      <Dialog
        open={rightSideDeleteDialogOpen}
        onOpenChange={setRightSideDeleteDialogOpen}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete Module</DialogTitle>
      ...
      <Dialog open={editWeekDialogOpen} onOpenChange={setEditWeekDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={handleEditWeekSave}>
              Save
            </Button>
            <DialogClose asChild>
              <Button variant="ghost">Cancel</Button>
      ...
      <Dialog
        ...
        onOpenChange={setDeleteWeekDialogOpen}
      >
        <DialogContent>
          <DialogHeader>
      ...
      <Dialog
        open={deleteWeekLessonDialogOpen}
        onOpenChange={setDeleteWeekLessonDialogOpen}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <div className="py-4">
            Are you sure you want to delete this? ID:{" "}
            {deleteWeekLessonData?.weekLessonId}
          </div>
          <DialogFooter className="flex justify-end gap-2">
            <Button
              variant="destructive"
              onClick={async () => {
                if (deleteWeekLessonData) {
                  await handleDeleteLessonFromWeek(
                    deleteWeekLessonData.weekId,
                    deleteWeekLessonData.weekLessonId
                  );
